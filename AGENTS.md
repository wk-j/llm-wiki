...
- In markdown **tables**, escape the `|` inside wikilink aliases as `\|` (e.g. `[[Codex\|Codex]]`), or drop the alias entirely (`[[Codex]]`). An unescaped pipe inside a table cell is parsed as a column separator and breaks the row.
