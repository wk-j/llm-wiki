...
## [2026-07-09] ingest | Bun in Rust
Ingested Jarred Sumner's Bun blog post "Rewriting Bun in Rust" (published 2026-07-08). Main frame: Bun's Rust rewrite is both a memory-safety migration and the strongest field report in the wiki so far for AI-assisted large-scale change. Key takeaways: (1) Bun's old Zig/C/C++ runtime had recurring stability bugs around GC/manual-memory boundaries; Rust was chosen for ownership/lifetime guarantees and `Drop`, not because Zig "failed" generally; (2) the rewrite deliberately started as a mechanical Zig-to-Rust port so maintainers could compare behavior side by side before later idiomatic refactors; (3) Jarred used Claude Code + pre-release Claude Fable 5 through ~50 dynamic workflows over 11 days, peaking around 64 Claudes, with 6,502 commits and a +1M-line landed diff; (4) the core safety pattern was `1 implementer + 2 adversarial reviewers + 1 fixer`, with reviewers in separate context and told to assume the code was wrong; (5) compiler errors became a work queue, but workflow rules had to reject stubs/workarounds and dangerous git commands; (6) the TypeScript test suite acted as a behavioral oracle across implementation languages; CI passed all platforms with zero tests skipped/deleted before merge; (7) source-attributed results: 128 v1.3.14 bugs fixed in v1.4.0, 19 known regressions fixed, lower memory use/binary size/stack usage, 2%-5% faster benchmarks, ~$165k API-pricing cost pre-merge. Preserved caveats: metrics come from Bun team; canary/release status should not be treated as independent benchmark.
- Created `[[bun-in-rust]]` (source), `[[adversarial-review-loops]]` (concept)
- Created entities: `[[bun]]`, `[[jarred-sumner]]`, `[[rust]]`, `[[zig]]`, `[[javascriptcore]]`, `[[prisma]]`
- Updated concepts: `[[dynamic-workflows]]`, `[[large-scale-changes]]`, `[[agentic-code-review]]`, `[[behavioral-verifier]]`, `[[git-worktrees]]`, `[[orchestration-tax]]`
- Updated entities: `[[anthropic]]`, `[[claude-code]]`, `[[fable]]`
- Updated `[[index]]`, `[[log]]`, and `hotcache.md`

## [2026-07-12] ingest | Everything to Know About GPT-5.6 and OpenAI Build Week
Ingested AIMeowYak / Piyalitt Ittichaiwong × Connected Human AI's YouTube live on GPT-5.6, GPT Live, Codex computer use, model comparisons, token economics, and OpenAI Build Week Bangkok. Main takeaways: (1) choose GPT-5.6 as `variant × effort × workload`, not by model name alone; the speakers recommend trying one effort step below GPT-5.5 for equivalent work, but this remains a field heuristic; (2) `ultra` can spawn many high-effort subagents and exhaust RAM/token, so maximum capability is not the default economical choice; (3) one-shot demos suggest GPT-5.6 improved at frontend/presentation/computer use while Fable 5 remained stronger for some Thai writing, but harness/effort differences prevent a universal ranking; (4) GPT Live combines low-latency speech interaction with parallel search/model calls, though Thai voice quality and memory continuity still lag; (5) computer use can automate authenticated browsing and paper downloads through the user's existing access, while high agency creates permission/cost risks; (6) equivalent intelligence may get cheaper even while top-end workflows cost more; (7) Build Week details and prize/speaker benefits remain source-attributed pending official confirmation. Preserved conflict with agentic-code-review/eternal-sloptember: the speakers report very few significant bugs from top models, while the wiki keeps verification and comprehension-risk evidence from other sources.
- Created source: `[[gpt-5-6-and-openai-build-week-aimeowyak]]`
- Created concept: `[[computer-use]]`
- Updated entities: `[[gpt-5-6-sol]]`, `[[piyalitt-ittichaiwong]]`, `[[openai-codex]]`
- Updated concepts: `[[effort-levels]]`, `[[ai-token-economics]]`
- Updated `[[index]]` and `hotcache.md`

## [2026-07-12] ingest | Wayfinder Skill
Ingested Matt Pocock's `wayfinder` SKILL.md (https://github.com/mattpocock/skills/blob/main/skills/engineering/wayfinder/SKILL.md). Main frame: plan work that is too large and foggy for one agent session by naming a destination, storing the effort as an issue map, and resolving one decision ticket per session. Key takeaways: (1) the map is an index, not a duplicate store; detail lives in the resolved ticket while `Decisions so far` keeps only a linked gist; (2) a precise question becomes a ticket even if unanswered, while an area whose question cannot yet be phrased stays in `Not yet specified` as fog; (3) out-of-scope work is beyond the destination and never graduates from fog; (4) open, unblocked, unclaimed child tickets form the frontier; assignee-as-claim plus native dependencies allow parallel sessions without collision; (5) ticket types are research (AFK), prototype (HITL), grilling (HITL), and task (AFK/HITL only to unblock a decision); (6) charting and ticket resolution are separate sessions, and no session resolves more than one ticket. Caveat preserved: this is a workflow specification from its author, not comparative team evidence, and it assumes tracker support or conventions for child issues and blocking.
- Created source: `[[wayfinder-skill]]`
- Created concept: `[[wayfinding]]`
- Updated entity: `[[matt-pocock]]`
- Updated concepts: `[[map-vs-territory]]`, `[[unknowns-matrix]]`, `[[queues-over-loops]]`, `[[grill-me]]`, `[[domain-modeling]]`
- Updated `[[index]]` and `hotcache.md`

## [2026-07-12] ingest | New Skills! v1.1
Ingested Matt Pocock's YouTube changelog "New Skills! v1.1 brings /wayfinder, /research, /implement, /to-spec, /to-tickets" (https://www.youtube.com/watch?v=A8mokin_YOs; transcript supplied by user). Main frame: Skills v1.1 turns Pocock's planning-oriented skills into a complete lifecycle: `grill/wayfind → to-spec → to-tickets → implement → code review → commit`. Key takeaways: (1) `/to-PRD` becomes `/to-spec` because the artifact is broader than a product requirements document; `/to-issues` becomes tracker-neutral `/to-tickets`; reinstall/check old skills during migration; (2) grilling asks one question at a time, distinguishes repo facts from user-owned decisions, and cannot implement before a shared-understanding confirmation gate; (3) `/implement` uses TDD at pre-agreed seams, regular type checks/single-test runs, one full-suite run at the end, then review and commit; (4) code review runs standards and spec-fidelity subagents in parallel and adds Martin Fowler refactoring-smell vocabulary; (5) Wayfinder sits before spec for work larger than one smart zone/session, with research/grilling/prototype/task tickets on a collaborative issue map; (6) `/research` uses a background agent and primary sources; (7) TDD becomes reference material focused on red-before-green, one slice at a time, while refactoring moves to review. Preserved tensions: `/to-spec` is not Specs-to-Code because human decision gates, tests and review remain; moving refactor out of the classic Red-Green-Refactor loop is Pocock's workflow preference, not a universal TDD definition. Usage metrics and effectiveness claims remain source-attributed.
- Created source: `[[new-skills-v1-1-wayfinder-research-implement-to-spec-to-tickets]]`
- Updated entity: `[[matt-pocock]]`
...
## [2026-07-16] ingest | Migel Tissera on X Android behavioral fingerprinting SDKs
Ingested Migel Tissera's X thread reporting two SDKs inside the X Android app that support process-wide behavioral fingerprinting. Main takeaways: (1) the reported signals include inter-keystroke timing and character deltas, clipboard event type/timing/length, touch and gesture events, a `MediaStore.Images` observer, motion sensors, device identifiers, carrier/root/emulator flags and a persistent XOR-obfuscated device token; (2) the author says the SDKs were inactive on his device, so the evidence supports dormant capability plus one runtime observation, not active collection across all users; (3) the author explicitly says clipboard text content was not collected, but does not define keyboard `character delta`, so both “harmless timing metadata” and “conventional keylogger” would overstate what the supplied text proves; (4) X Money/X Pay, ID verification and anti-fraud are the author's plausible hypothesis, not a confirmed purpose from X; (5) process-wide scope still raises proportionality, consent, retention and false-positive questions even if the purpose is legitimate; (6) the attached session log and promoted `apksaw` MCP tool make independent reproduction possible in principle, but the supplied text lacks full APK version/hash, SDK names, activation config and network trace. Preserved unrelated Meta/TikTok claims by treating comparisons in the conversation as speculation rather than updating those pages.
- Created source: `[[migel-tissera-x-android-behavioral-fingerprinting]]`
- Created entities: `[[migel-tissera]]`, `[[x-twitter]]`
- Created concepts: `[[behavioral-biometrics]]`, `[[mobile-app-reverse-engineering]]`
- Updated concept: `[[learning-from-users]]` to separate behavioral risk scoring from learning a user's corrective decision
- Updated `[[index]]`, `[[log]]`, and `hotcache.md`

## [2026-07-17] ingest | Kimi K3 Explained!
Ingested Prompt Engineering's YouTube transcript on Kimi K3. Main takeaways: (1) the video's strongest claim is that K3 reaches the frontier in specialized agentic coding, not that it wins general chat; (2) the speaker cites strong Terminal-Bench 2.1/web development results while saying K3 trails Fable 5 on HLE and some Opus-class model on hallucination rate; (3) architecture claims are 3T total parameters, 896 MoE experts with 16 routed per token, native vision and 1M context; (4) open weights do not imply local accessibility because a 3T model still needs enterprise-scale serving; (5) Kimi's value to the ecosystem may be as a post-training base, but Cursor/Cognition-Windsurf/Thinking Machines examples remain unresolved because the transcript garbles names and lineage; (6) pricing should be measured per accepted task after token, retries, latency, review and rework. Preserved the internal conflict between calling K3 open-weights and saying “when the weights are available”; technical, benchmark and pricing claims remain source-attributed pending primary-source verification.
- Created source: `[[kimi-k3-explained-prompt-engineering]]`
- Created entity: `[[kimi-k3]]`
...
## [2026-08-15] ingest | Software Is Made Between Commits — Nathan Sobo / Zed
Ingested Nathan Sobo's Zed launch post introducing DeltaDB as version control for the work that happens between commits. Main takeaways: (1) Git captures discrete commit snapshots while DeltaDB gives every fine-grained operation a stable delta identity; (2) agent messages and their edits are versioned together so references survive line movement and users can navigate conversation→code at that moment/currently and code→all conversations that touched it; (3) CRDT-backed replicated worktrees let people and agents edit real files across machines through terminal or mounted disk, but conflict-free replication does not remove semantic conflicts; (4) Sobo wants teammates to collaborate with the agent before commit/push, while Git and CI remain the integration/check layer; (5) the article announces beta in “a few weeks” without a publication date in the supplied text and gives no implementation, performance, privacy, retention, access-control, recovery or Git-interoperability details, so those claims remain first-party and open. Preserved three tensions instead of overwriting prior pages: `[[zed-is-1-0]]` called DeltaDB a sync engine/foundation already present around Zed 1.0 while the new article calls the user-facing product version control with beta still ahead; Git worktree isolation and DeltaDB shared worktrees optimize different collaboration shapes; stacked PR review may remain the integration/audit gate even if live discussion moves before commit.
- Created source: `[[software-is-made-between-commits]]`
- Created entity: `[[nathan-sobo]]`
- Created concepts: `[[conversation-code-provenance]]`, `[[crdts]]`
- Updated entities: `[[deltadb]]`, `[[zed]]`, `[[zed-industries]]`
- Updated concepts: `[[collaborative-ai-engineering]]`, `[[git-worktrees]]`, `[[stacked-pull-requests]]`
- Updated `[[index]]`, `[[log]]`, and `hotcache.md`

## [2026-08-16] ingest | Claude Now Watermarks Its Text — Squintist
Ingested Squintist's YouTube video on Anthropic's August 2026 support page announcing invisible text watermarking in every new Claude model. The durable frame splits three things that get conflated: (1) sampling-time watermarks — SynthID-style tournament sampling keyed on a secret plus recent context, statistically invisible (Google's ~20M-response Gemini experiment: 0.01% thumbs-up difference, ~0.5% slowdown) but weak on low-entropy text and washable by paraphrase (58/59 removal on a public MarkLLM reimplementation over Gemma-2-9B, not Google's production detector); (2) style-based AI detectors — no key, guessing from style, with a documented false-positive record (OpenAI's withdrawn 2023 classifier, 61% average FP on non-native TOEFL essays, a professor's hand-polished comedy story flagged 100% AI, NeurIPS 2026 desk-rejecting 178 of 969 position papers via Pangram with no appeal) and a dissolving premise as humans absorb model vocabulary; (3) what a mark means — Anthropic's own wording says "processed by Claude," covering proofreading, translation, summarizing and writing alike, so it is provenance, not authorship or quality. Kept open: Anthropic has not published its mechanism (the SynthID-family inference is the video's), the EU-law tension (Article 50 exempts standard editing but Claude marks copy-edits anyway), vendor-authored accuracy claims from Pangram/GPTZero, and the enforcement gap that self-hosted open-weight models cannot be forced to watermark.
- Created source: `[[claude-text-watermarking-squintist]]` (raw saved to `raw/claude-text-watermarking-squintist.md`)
- Created concepts: `[[llm-text-watermarking]]`, `[[ai-text-detectors]]`
- Created entities: `[[synthid]]`, `[[eu-ai-act]]`, `[[pangram]]`, `[[squintist]]`
- Updated entities: `[[anthropic]]` (watermark announcement + code-of-practice signature), `[[claude]]` (family-wide watermark note), `[[google-deepmind]]` (SynthID section), `[[openai]]` (withdrawn 2023 detector, image-only provenance, EU code signature)
- Updated concepts: `[[quality-proxy-collapse]]` (fluent prose and "too smooth = machine" both collapsing; watermark as the narrow replacement proxy), `[[open-weight-models]]` (watermark enforcement gap)
- Preserved tensions: watermark = "touched by model" vs reader intuition "authored by model"; EU standard-editing exemption vs Anthropic marking copy-edits; open-source SynthID transparency vs giving attackers a test bench; detector accuracy claims vendor-attributed on both directions
- Updated `[[index]]`, `[[log]]`, and `hotcache.md`
