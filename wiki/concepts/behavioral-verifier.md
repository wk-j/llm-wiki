...
created: 2026-05-27
updated: 2026-07-09
sources: [Piyalitt Ittichaiwong - DeepSWE FrontierSWE Benchmark.md, Agentic Code Review.md, https://openai.com/index/introducing-genebench-pro/, bun-in-rust.md]
---
...
## ข้ามจาก code ไป science

[[genebench-pro|GeneBench-Pro]] ทำให้เห็นว่า verifier ในงานวิทยาศาสตร์ต้องตรวจมากกว่า output สุดท้าย. ใน computational biology agent อาจได้เลขใกล้เป้าแต่เลือก causal method ผิด, ไม่จัดการ data quality issue, หรือใช้ estimand ที่ data รองรับไม่ได้

OpenAI เลยสร้างโจทย์แบบ synthetic ที่รู้ causal structure และใช้ ablation ตรวจว่า analysis ที่ plausible แต่ผิดควร fail. นี่คือ behavioral verifier ในระดับ scientific analysis: ตรวจว่า agent เดินทางวิเคราะห์ถูกพอจะเชื่อผล ไม่ใช่แค่ format คำตอบถูก

## ข้ามภาษา: Bun ใช้ test suite เดิมเป็น oracle

[[bun-in-rust|Bun in Rust]] เป็นตัวอย่าง verifier ในงาน production rewrite. [[bun|Bun]] ย้ายจาก [[zig|Zig]] เป็น [[rust|Rust]] แต่ test suite หลักเขียนด้วย TypeScript จึงไม่สนว่า implementation language คืออะไร. มันตรวจพฤติกรรมของ runtime/toolchain จากมุมผู้ใช้.

ก่อน merge ทีมรัน CI ทุก platform และ Jarred ตรวจเองว่าไม่มี test ถูก skip หรือ delete. ตรงนี้คือ behavior verifier ในชีวิตจริง: ถ้า verifier ผูกกับ Zig implementation เดิม rewrite จะใช้ไม่ได้ แต่เพราะมันตรวจ behavior ของ Bun จึงใช้เป็น oracle ข้ามภาษาได้.

**ได้อะไร:** test suite ที่ดีเป็น asset เชิงยุทธศาสตร์. มันทำให้ rewrite ใหญ่, agent workflow, หรือ model migration เป็นไปได้โดยไม่ต้องเชื่อใจ diff อย่างเดียว.
