...
created: 2026-04-18
updated: 2026-06-15
sources: [harness-engineering-panutat.md, llm-era-computer-engineering-nattee.md, alex-ker-harnesses-optimize.md, thclaws-announcement-panutat.md, Agent Harness Engineering.md, "Introducing Omnigent A Meta-Harness to Combine, Control and Share Your Agents.md", Self Learning for Agents Clearly Explained.md]
---
...
## Harness ที่เขียนตัวเองได้ (self-rewriting harness)

ในกรอบ [[three-learning-layers|3 ชั้นการเรียนรู้]] ของ [[ataiiam|@ataiiam]], harness ไม่ใช่แค่สิ่งที่ *คนออกแบบ* แต่เป็นชั้นที่ **เรียนรู้ปรับตัวเองได้** — และนี่คือหลักฐานที่ชัดที่สุดว่า "Agent = Model + Harness" จริง:

- **LangChain Deep Agents** — รัน agent กับงาน batch, เก็บ trace, ชี้ coding agent ไปหา failure pattern แล้ว **เขียน prompt/tool/hook ใหม่ โดยตรึง model ไว้** ได้จาก 52.8 → 66.5 บน Terminal-Bench 2.0 (top 30 → top 5); agent เสนอ คนอนุมัติ
- **Self-Harness** — loop เดียวกันแต่ไม่มีคนดู: หา failure → เสนอการแก้ → test → เก็บถ้าดีขึ้น. ผลที่ตอกย้ำ: วิธีเดียวกันยก **3 model ที่ตรึง weight** ได้หมด (40.5→61.9, 23.8→38.1, 42.9→57.1)

> ได้อะไร: เมื่อเปลี่ยน *แค่ harness* แล้วทุก model ดีขึ้น แปลว่า harness คือสิ่งที่รั้ง model ไว้ ไม่ใช่ความฉลาดของ model. นี่คือเหตุผลเชิงประจักษ์ว่าทำไม harness engineering คุ้มกว่ารอ model ใหม่ — ต่อยอดหลัก [[harness-ratchet]] ที่ failure ต้องกลายเป็น control ถาวร แค่ตอนนี้ตัว agent ทำ ratchet นั้นได้เองบางส่วน. ข้อจำกัด: ทั้งคู่รันได้แค่ที่มี free scorer (test/benchmark) ส่วนงานที่ไม่มีต้องพึ่งคน ดู [[harness-guides-sensors]].

## Meta-Harness: ยก control ขึ้นเหนือหลาย harness

[[omnigent|Omnigent]] เพิ่มขอบเขตใหม่ให้ harness engineering: ถ้า workflow ใช้ Claude Code, Codex, Pi และ custom agent พร้อมกัน policy กับ collaboration ไม่ควรถูกขังอยู่ใน harness ตัวใดตัวหนึ่ง. [[meta-harness|Meta-harness]] จึงทำหน้าที่เป็น control plane กลางสำหรับ composition, contextual policy, cost, sandbox และ live session sharing.

มุมนี้ทำให้ harness engineering ไม่ได้หยุดที่ "ออกแบบ agent ตัวหนึ่งให้ดี" แต่รวมถึง "ออกแบบกติกาที่เดินทางข้าม agent หลายชนิด". จุดเสี่ยงคือ control plane กลางจะกลายเป็นระบบสำคัญมาก ความผิดพลาดจุดเดียวอาจกระทบ agent ทั้ง fleet.

**ผลคือ:** governance ใช้ซ้ำได้มากขึ้น แต่ต้อง harden และ audit ชั้นกลางให้จริงจังกว่าเดิม.
