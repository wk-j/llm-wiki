...
**Peter Steinberger** (ชื่อเล่นในวงการ: **the Clawfather**) คือผู้สร้าง **OpenClaw** ระบบ agent ที่ผลักขอบเขตของงาน agent จน OpenAI ต้องขยับตาม. ตอนนี้เขาร่วมงานกับ [[openai|OpenAI]] แล้ว และขึ้นเวที keynote ของทีม [[openai-codex|Codex]] ใน [[ai-engineer-worlds-fair|AI Engineer World's Fair]] (สรุปใน [[piyalitt-codex-keynote-attention-not-token]])

## ทำไมถึงน่าฟัง

Peter เป็นตัวอย่าง power user ที่เปลี่ยนวิธีทำงานเร็วกว่า harness ของตลาด. เขาเริ่มจากการเปิด 10+ terminal แล้วคอย polling เอง ก่อนจะย้ายไปใช้ **manager agent ตัวเดียว** ที่กระจายงานให้ worker. ประโยคที่ wiki อ้างบ่อยจากเขา:
...
- คุยกับ **manager agent** ตัวเดียวที่รันต่อเนื่องระยะยาว แล้วให้มันกระจายงานไปทีม worker
- งานไหนยากจริงค่อยลงไป pair กับ worker โดยตรง
- เขาสรุปภาพนี้ว่า: *บริหารผู้จัดการของบริษัทเล็ก ๆ ที่พนักงานเป็น agent*

สามอย่างที่ทำให้ loop นี้ทำงานได้:
...
ใน open source project ของเขา งานไหลแบบนี้: issue เข้ามา → manager ตัดสินใจ → worker สืบ/แก้/test → agent รีวิวอีกชั้น → ส่ง PR กลับเมื่อต้องการ Peter จริง ๆ → Peter รีวิวรอบเดียว อนุมัติหรือทิ้งโน้ต → loop หมุนต่อ. ดู [[inner-loop-outer-loop]]

## โจทย์ที่ยังเปิด

Peter บอกตรง ๆ ว่ายังไม่มีรูปแบบสุดท้ายสำหรับ manager agent ที่:

- ไม่ผูกกับโน้ตบุ๊กเครื่องเดียว
- เชื่อมทุกเครื่องของเขาเอง และรู้เองว่างานไหนควรไป cloud งานไหนต้องอยู่ local
