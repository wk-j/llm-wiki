...
## เคส Bun rewrite (Jarred Sumner, 2026-07)

[[bun-in-rust|Bun in Rust]] เป็น source ที่ใช้ Fable ในงาน production ใหญ่ที่สุดใน wiki ตอนนี้. [[jarred-sumner|Jarred Sumner]] บอกว่าใช้ pre-release Fable 5 เป็นส่วนสำคัญในการ rewrite [[bun|Bun]] จาก [[zig|Zig]] เป็น [[rust|Rust]] ผ่าน [[claude-code|Claude Code]] และ workflow ขนาน.

จุดสำคัญคือ Fable ไม่ได้ถูกใช้เป็น oracle เดี่ยว. มันอยู่ใน harness ที่มี porting guide, lifetime table, [[adversarial-review-loops|adversarial review loops]], compiler/test/CI/fuzzing และ human monitoring. นี่ทำให้หน้า Fable มีอีกแกนหนึ่ง: จาก "model ที่ใช้ judgement ดี" ไปสู่ "model ที่ทำงานได้เมื่อถูกวางใน workflow ที่ proof-driven".

## ถูกวางเป็นคู่เทียบของ GPT-5.6 Sol

[[gpt-5-6-sol-fable-killer-prompt-engineering|วิดีโอของ Prompt Engineering]] ใช้ Fable 5 เป็นเส้นมาตรฐานของ frontier coding แล้วถามว่า [[gpt-5-6-sol|GPT-5.6 Sol]] ฆ่า Fable ได้หรือยัง. ผู้พูดอ้างว่า Sol สูสีหรือชนะ coding benchmark บางตัวด้วยต้นทุนต่ำกว่า แต่ทักเองว่าการตั้ง effort ของฝั่ง Claude อาจไม่ถึงระดับสูงสุด และเขายังไม่ได้ลอง Sol โดยตรง.

จุดนี้ไม่ลบหลักฐานเดิมที่ Fable ทำงานได้ดีใน field guide, Elysia audit หรือ Bun rewrite. มันเพิ่มคำถามใหม่ว่า เมื่อคุม prompt, harness, effort, budget และเวลารันให้เท่ากัน Sol จะยังได้เปรียบหรือไม่.

## ถูกใช้เป็นเส้นเทียบของ Kimi K3

[[kimi-k3-explained-prompt-engineering|วิดีโอ Kimi K3]] วาง [[kimi-k3|K3]] เป็น specialist ที่อาจชนะ Fable-class models ด้าน coding/web development แต่ตามหลัง Fable 5 มากใน Humanity's Last Exam. นี่ไม่หักล้าง field reports เดิมของ Fable เพราะแต่ละแหล่งวัดคนละ workload และใช้ harness ต่างกัน.

คำถามที่เพิ่มเข้ามาคือ ถ้าคุม terminal task, budget, tool, effort และ stop condition เท่ากัน K3 ยังชนะด้าน agentic coding หรือไม่ ขณะเดียวกัน Fable ยังนำด้าน general reasoning แค่ไหน.
